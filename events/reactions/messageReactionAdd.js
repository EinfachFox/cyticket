...
if(emoji.toString() === config.emoji){
	...
	} else role = message.guild.roles.cache.find(r => r.name === "CYTICKET");

	if(!role) {
		return await message.guild.roles.create({data:{name: "CYTICKET", permissions: 0}, reason: 'Ticket Support Role'});
	  };
	...
	if(array.length >= limit){
		return message.channel.send(`Ticketlimit erreicht. Grenze: ${limit}`).then((s)=>[
      ...
      message.guild.channels.create(chann, { permissionOverwrites:[
		...
		.setDescription(`Willkommen zu Ihrem Ticket ${user}!\n\nBitte erläutern Sie unten Ihr Problem.`)

		const closeEmbed = new discord.MessageEmbed()
		.setColor(message.client.color.red)
		.setDescription(`Bitte reagiere mit 🗑️ um das Ticket zu schließen.`)
