  ...
  async run(message) {
    try {
     ...
     if (message.content.match(mentionRegex)) {
       ...
       message.channel.send(`Hallo! Mein Präfix für diesen Server ist ${config.prefix}`).then((s)=>{
      ...
      if (command) {
        ...
        if (typeof rateLimit === "string") return message.channel.send(` ${message.client.emoji.fail} Warten Sie mal **${rateLimit}** vor dem Ausführen der **${cmd}** erneut befehlen - ${message.author}`);
      ...
      message.channel.permissionsFor(message.guild.me).missing(command.botPermission).map(p => permissions[p]);
       ...
       const embed = new MessageEmbed()
        ...
        .setTitle(`${message.client.emoji.fail} Fehlende Bot-Berechtigungen`)
        .setDescription(`Befehlsname: **${command.name}**\nErforderliche Erlaubnis: **${missingPermissions.map(p => `${p}`).join(' - ')}**`)
        .setTimestamp()
      ...
      if (missingPermissions.length !== 0) {
        const embed = new MessageEmbed()
          ...
          .setTitle(`${message.client.emoji.fail} Fehlende Benutzerberechtigungen`)
          .setDescription(`Befehlsname: **${command.name}**\nErforderliche Erlaubnis: **${missingPermissions.map(p => `${p}`).join('\n')}**`)
          .setTimestamp()
          .setFooter('https://pogy.xyz')
          .setColor(message.guild.me.displayHexColor);
       return message.channel.send(embed).catch(()=>{})
        ...
        if (command.disabled) return message.channel.send(`Der Eigentümer hat den folgenden Befehl vorerst deaktiviert.`)
        ...
        .catch(error => {
           console.log(error)
     return message.channel.send(`Ein Fehler ist aufgetreten, bitte informieren Sie den Entwickler.`)
        })
      }
    } catch(error) {
      console.log(error)
    ...
    async runCommand(message, cmd, args) {
        ...
        if (!message.channel.permissionsFor(message.guild.me) || !message.channel.permissionsFor(message.guild.me).has('EMBED_LINKS'))
          return message.channel.send(`${message.client.emoji.fail} Bot-Berechtigungen fehlen - **Bindet Links ein**`)
    ...
    ratelimit(message, cmd) {
      try {
        ...
        console.log(e)
        message.channel.send(`Ein Fehler ist aufgetreten, bitte informieren Sie den Entwickler.`)
      }
    }
}
